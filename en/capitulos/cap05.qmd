...
![A keyless hash function detects accidental errors, but does not withstand an active attacker: since anyone can compute $H$, an attacker who alters the data can also recompute a new, valid hash, and the receiver has no way of telling this situation apart from a legitimate communication.](../assets/images/fig-05-hash-integridade-ataque.png){#fig-05-integridade-ataque width=100%}
...
![The HMAC construction: the message is first combined with one version of the key ($K \oplus \text{ipad}$) and passed through $H$, producing an inner hash; this result is then combined with a second version of the key ($K \oplus \text{opad}$) and passed through $H$ a second time, producing the final tag. Neither invocation of $H$ ever processes the original key directly.](../assets/images/fig-05-hmac-construcao.png){#fig-05-hmac-construcao width=95%}
...
## Food for Thought {#sec-05-reflexao}
