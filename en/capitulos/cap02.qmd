...
![One round of a Feistel network, illustrated for a block of $W=64$ bits (the notation $LE_i$/$RE_i$ used in the figure is equivalent to $L_i$/$R_i$). The right half $RE_0$ passes through a function $F$ parameterised by the round subkey $K_1$, the result is combined by XOR with the left half $LE_0$, and the two halves swap position for the next round ($LE_1$/$RE_1$).](../assets/images/fig-02-feistel-ronda.png){#fig-02-feistel-ronda width=45%}
...
![Encryption and decryption in a 16-round Feistel network (the number of rounds used by DES, presented in the next section). On the left, encryption: the plaintext enters as $LE_0$/$RE_0$ and, round by round, the subkeys $K_1, K_2, \dots, K_{16}$ are applied in order, producing the ciphertext. On the right, decryption starts from that same ciphertext and applies exactly the same round structure, with the same function $F$, but with the subkeys in reverse order ($K_{16}$ in the first round of decryption, $K_1$ in the last), recovering the original plaintext after the 16 rounds.](../assets/images/fig-02-feistel-enc-dec-en.png){#fig-02-feistel-enc-dec width=70%}
