...
![The symmetric cipher model: sender and receiver have previously shared the same secret key, over a secure channel. The encryption algorithm uses that key to transform plaintext into ciphertext; the decryption algorithm, using the same key, reverses the transformation.](../assets/images/fig-modelo-cifra-simetrica-en.png){#fig-01-modelo-simetrico}
...
![Letter frequency in English, as standardly published in the cryptographic literature [@Stallings2020].](../assets/images/fig-frequencia-letras-en.png){#fig-01-frequencia-letras}
...
Comparing this table with the reference chart (@fig-01-frequencia-letras): P, the most frequent ciphertext letter by a wide margin, is the natural candidate to correspond to E, the most frequent letter of English (12.7%); Z, the second most frequent, is the natural candidate for T (9.1%), the second most frequent letter of the language. From here, a cryptanalyst continues the same reasoning across the rest of the alphabet, cross-checking each new hypothesis against likely short words and digraphs, exactly the technique already used above with "to" and "the" — with enough ciphertext, as here, the process converges on the complete substitution key. This is a statistics-based cryptanalysis: the longer the message, the more reliable the process.
...
![Playfair grid built from the keyword "ESIGELEC".](../assets/images/fig-playfair-grelha-en.png){#fig-01-playfair-grelha width=44%}
...
![The Vernam cipher: a keystream generator produces the sequence $k_i$, combined by XOR with the plaintext during encryption, and with the ciphertext during decryption — the same operation in both directions.](../assets/images/fig-vernam-gerador-en.png){#fig-01-vernam-gerador width=95%}
...
![The rail fence technique: the plaintext is written in a zigzag across two lines; the ciphertext is read line by line.](../assets/images/fig-rail-fence-en.png){#fig-01-rail-fence width=80%}
...
![Columnar transposition grid with key 4312567. The digits above each column give the order in which the columns are read out.](../assets/images/fig-transposicao-grelha-en.png){#fig-01-transposicao-grelha width=68%}
...
6. **Transposition ciphers** reorder the symbols of the plaintext without changing their identity, and form the historical basis of the modern concepts of confusion and diffusion
