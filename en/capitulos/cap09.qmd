...
![The attacker (Mallory) intercepts Alice's and Bob's public keys, and substitutes them with her own before forwarding them on. Each party thinks they are using the other's public key, when in fact they are using Mallory's public key.](../assets/images/fig-09-mitm-en.png){#fig-09-mitm width=85%}
...
![Essential structure of an X.509 certificate: the holder's data (identity, public key, validity period, issuer, and serial number), and the certificate authority's signature over the hash of that data.](../assets/images/fig-09-x509-en.png){#fig-09-x509 width=55%}
...
![The certificate chain: the Root CA signs certificates for intermediate CAs, which in turn sign further certificates, down to the final holder of a domain. Trusting the root, already installed on the system, is enough to validate the entire chain.](../assets/images/fig-09-cadeia-ca-en.png){#fig-09-cadeia-ca width=85%}
...
## Discussion Question {#sec-09-reflexao}
