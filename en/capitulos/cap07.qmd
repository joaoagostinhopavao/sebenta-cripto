...
![The Diffie-Hellman algorithm: $g$ and $p$ are public; $x$ and $y$ never leave the computer of whoever chose them; only $A$ and $B$ cross the open channel. Both sides arrive at the same secret $K=g^{xy} \bmod p$, without it ever having been transmitted.](../assets/images/fig-07-diffie-hellman.png){#fig-07-dh width=85%}
...
![Alice signs $m$ with her private key and the ephemeral value $k$, producing $(r,s)$; anyone with Alice's public key can then verify that the signature is valid, without ever having access to $x$ or $k$.](../assets/images/fig-07-dsa-assinar-verificar.png){#fig-07-dsa-verificar width=100%}
...
## Food for Thought {#sec-07-reflexao}
