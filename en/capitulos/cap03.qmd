...
![The AES state: the 128-bit block (16 bytes) is organised into a $4\times4$ matrix, filled column by column (byte 0 to byte 3 in the first column, byte 4 to byte 7 in the second, and so on).](../assets/images/fig-aes-estado.png){#fig-03-aes-estado width=45%}
...
![In the SubBytes step, each byte of the state is replaced by the corresponding byte in a fixed S-box. Source: Wikimedia Commons, public domain.](../assets/images/fig-AES-SubBytes.png){#fig-03-subbytes width=60%}
...
![In the ShiftRows step, each row of the state matrix is cyclically shifted to the left, by a different number of positions per row. Source: Wikimedia Commons, public domain.](../assets/images/fig-AES-ShiftRows.png){#fig-03-shiftrows width=75%}
...
![In the MixColumns step, each column of the state matrix is multiplied by a fixed polynomial $c(x)$, mixing its four bytes. Source: Wikimedia Commons, public domain.](../assets/images/fig-AES-MixColumns.png){#fig-03-mixcolumns width=75%}
...
![In the AddRoundKey step, each byte of the state is combined by XOR with the corresponding byte of the round's subkey. Source: Wikimedia Commons, public domain.](../assets/images/fig-AES-AddRoundKey.png){#fig-03-addroundkey width=60%}
...
![The complete structure of AES: an initial `AddRoundKey`, followed by $N_r-1$ intermediate rounds (`SubBytes`, `ShiftRows`, `MixColumns`, `AddRoundKey`), and a final round that omits `MixColumns`.](../assets/images/fig-aes-rondas-estrutura.png){#fig-03-rondas-estrutura width=75%}
...
![The ECB mode: each plaintext block is encrypted (or decrypted) completely independently of the others, always with the same key $K$ — hence the absence of any link between consecutive blocks.](../assets/images/fig-modo-ecb.png){#fig-03-modo-ecb width=90%}

![A plaintext organised into blocks, with an obvious structural pattern (frame, cross), encrypted in ECB mode and in CBC mode. Under ECB, identical plaintext blocks generate identical ciphertext blocks: the original pattern remains perfectly visible, only with different colours (values). Under CBC, the same pattern disappears completely.](../assets/images/fig-ecb-vs-cbc.png){#fig-03-ecb-cbc width=95%}
...
![The CTR mode: each counter $T_i = \text{nonce} \Vert i$ is encrypted independently of the others, and the result is combined by XOR with the corresponding plaintext block (top) or ciphertext block (bottom) — encryption and decryption use exactly the same operation, `Block Cipher Encryption`.](../assets/images/fig-modo-ctr.png){#fig-03-modo-ctr width=95%}
...
## Food for Thought {#sec-03-reflexao}
